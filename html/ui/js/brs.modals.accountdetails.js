...
var BRS = (function(BRS, $, undefined) {
	...
	if (BRS.accountInfo.errorCode && BRS.accountInfo.errorCode != 5) {
	    ...
	    if (BRS.accountInfo.errorCode && BRS.accountInfo.errorCode == 5) {
		$("#account_balance_balance, #account_balance_unconfirmed_balance, #account_balance_effective_balance, #account_balance_guaranteed_balance").html("0 Peth");
		...
		$("#account_balance_account").html(String(BRS.account).escapeHTML());
				...
				var balanceNQT=	BRS.formatAmount(new BigInteger(BRS.accountInfo.balanceNQT))
			    var balanceNQT=String(balanceNQT.replace(/,/g,'')*10000)

			    var unconfirmedBalanceNQT=BRS.formatAmount(new BigInteger(BRS.accountInfo.unconfirmedBalanceNQT))
			    var unconfirmedBalanceNQT=String(unconfirmedBalanceNQT.replace(/,/g,'')*10000)

			    var committedBalanceNQT=BRS.formatAmount(new BigInteger(BRS.accountInfo.committedBalanceNQT))
			    var committedBalanceNQT=String(committedBalanceNQT.replace(/,/g,'')*10000)
		$("#account_balance_balance").html(balanceNQT+ " Peth");
		$("#account_balance_unconfirmed_balance").html( unconfirmedBalanceNQT+ " PETH");
                $("#account_locked_balance").html(BRS.formatAmount(balanceNQT - unconfirmedBalanceNQT.toLocaleString()) + " Peth");
		$("#account_committed_balance").html(committedBalanceNQT + " PETH");
