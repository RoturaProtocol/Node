...
public class SqlBlockDb implements BlockDb {
  ...
  public Block findBlock(long blockId) {
    ...
    JsonObject jsonObject = connection.get(String.valueOf(blockId)).contentAsObject();

    try {
      Block block = Block.parseBlock(JsonParser.parseString(jsonObject.toString()).getAsJsonObject());

      return jsonObject == null ? null : block;
    } catch (Exception e) {
...
//    });
  ...
  public boolean hasBlock(long blockId) {
    Long id = null;
    try {
      id = connection.get(String.valueOf(blockId)).contentAsObject().getLong("id");
    }catch (Exception e){
      return false;
    }
    if(id == null){
...
//    });
  ...
  public long findBlockIdAtHeight(int height) {
    String query = String.format("SELECT id FROM `%s` WHERE height = %d", bucketName,height);
...
//    });
  ...
  public Block findBlockAtHeight(int height) {
    String query = String.format("SELECT a.* FROM `%s` a WHERE height = %d", bucketName,height);
    List<JsonObject> result = cluster.query(query).rowsAsObject();
    if (result.isEmpty()) {
      throw new RuntimeException("Block at height " + height + " not found in database!");
    }
    ...
    try {
       ...
       block = Block.parseBlock(JsonParser.parseString(result.get(0).toString()).getAsJsonObject());
    } catch (BurstException.ValidationException e) {
      throw new RuntimeException("findBlockAtHeight", e);
    }
    return block;
...
//    });
  ...
  public Block findLastBlock() {
    ...
    String query = String.format("SELECT a.* FROM `%s` a order by height desc limit 1", bucketName);
    List<JsonObject> result = cluster.query(query).rowsAsObject();
    if (result.isEmpty()) {
    ...
    try {
      ...
      throw new RuntimeException("findLastBlock", e);
    }
    return block;
...
//    });
  ...
  public Block findLastBlock(int timestamp) {
    String query = String.format("SELECT a.* FROM `%s` a WHERE timestamp <= %d ORDER BY height DESC LIMIT 1", bucketName, timestamp);
    List<JsonObject> result = cluster.query(query).rowsAsObject();
    if (result.isEmpty()) {
      throw new RuntimeException("Block at timestamp " + timestamp + " not found in database!");
    }
    Block block = null;
    try {
      block = Block.parseBlock(JsonParser.parseString(result.get(0).toString()).getAsJsonObject());
...
//  }
  ...
  public void saveBlock(Block block) {
    System.out.println("saveblock");
    System.out.println(block.getId());
    System.out.println(block.getJsonObject());
    com.google.gson.JsonObject jsonObject = block.getJsonObject();
    Gson gson = new Gson();
    String jsonString = gson.toJson(jsonObject); // 将 Gson JsonObject 转换为 JSON 字符串
    com.couchbase.client.java.json.JsonObject couchbaseJsonObject = com.couchbase.client.java.json.JsonObject.fromJson(jsonString);
    connection.upsert(String.valueOf(block.getId()),couchbaseJsonObject);
    Burst.getDbs().getTransactionDb().saveTransactions(block.getTransactions());
    if (block.getPreviousBlockId() != 0) {

      String query = String.format("UPDATE `%s`  SET nextBlockId = %d  WHERE id = %d", bucketName, block.getId(),block.getPreviousBlockId());
      cluster.query(query);
...
//  }
  ...
  public void deleteBlocksFrom(long blockId) {
    String query = String.format("SELECT height FROM `%s` WHERE id = %d", bucketName, blockId);
    QueryResult selectResult = cluster.query(query);
    Integer blockHeight = selectResult.rowsAsObject().isEmpty() ? null : selectResult.rowsAsObject().get(0).getInt("height");

    if (blockHeight != null) {
      String deleteQuery = String.format("DELETE FROM `%s` WHERE height >= %d", bucketName, blockHeight);
